...
public class DatabaseManager {
    ...
    public Connection getConnection() throws SQLException {
        ...
        // Optional but recommended: set pragmas per connection
        Connection conn = DriverManager.getConnection(url);
        try (Statement s = conn.createStatement()) {
            s.execute("PRAGMA foreign_keys = ON");
            s.execute("PRAGMA busy_timeout = 5000"); // 5s lock wait
    ...
    public void initialize() {
        ...
        this.plugin.getLogger().info("Database path: " + this.dbPath);

        try (Connection conn = getConnection()) {
            this.plugin.getLogger().info("Database connection successful");
            createTables(conn);
        } catch (SQLException e) {
            plugin.getLogger().severe("Failed to connect/create tables!");
            e.printStackTrace();
        }
    }

    public void close() {
        // no-op with open-per-operation
        plugin.getLogger().info("Database manager closed (no pooled connections).");
    }

    private void createTables(Connection conn) {
        String arenasTable = "CREATE TABLE IF NOT EXISTS arenas (" +
        ...
        String matchResultsTable = "CREATE TABLE IF NOT EXISTS match_results (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT," +
