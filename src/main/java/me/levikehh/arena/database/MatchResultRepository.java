...
public class MatchResultRepository {
     ...
     */
    private void updatePlayerStats(Player player, MatchResult matchResult) {
        // First, ensure player exists in stats table
        String insertOrIgnore = "INSERT OR IGNORE INTO player_stats " +
        ...
        // Update stats
        boolean isWinner = matchResult.getWinner() != null && 
                          matchResult.getWinner().equals(player);
        boolean isLoser = matchResult.getLoser() != null && 
                         matchResult.getLoser().equals(player);
        ...
        String sql = "UPDATE player_stats SET " +
                    ...
                    "player_name = ?, " + // Update name in case it changed
        ...
        try (Connection conn = databaseManager.getConnection();
            ...
            pstmt.setInt(2, isLoser ? 1 : 0);
            pstmt.setInt(3, isDraw ? 1 : 0);
            pstmt.setInt(4, (isWinner && matchResult.getType() == MatchResult.ResultType.KILL) ? 1 : 0);
            pstmt.setInt(5, isLoser ? 1 : 0);
            pstmt.setInt(6, matchResult.getDuration());
            pstmt.setString(7, player.getName());
            pstmt.setString(8, player.getUniqueId().toString());
